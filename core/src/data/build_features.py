...
def split(
    filepath: str, n_use: int, target: str, train_size: float, random_state: int
...
def _embed(model: SentenceTransformer, text: str, chunk_size: int, overlap: int) -> np.ndarray:
    """
...
def create_features(
    ...
    for name, df in zip(("train", "test"), (X_train, X_test)):
        for col in columns:
            ...
            df[f"mean_word_length_{col}"] = df[col].apply(lambda s: mean_word_length(s))
            df[f"max_word_length_{col}"] = df[col].apply(lambda s: max_word_length(s))
            df[f"num_digits_{col}"] = df[col].apply(lambda s: num_digits(s))
            df[f"num_punctuation_{col}"] = df[col].apply(lambda s: num_punctuation(s))
            to_keep[name] += [
                f"length_{col}",
                f"num_words_{col}",
...
def reducer(
    X_train: pd.DataFrame, X_test: pd.DataFrame, product_id: str, n_components: int
    ...
    X_train_reduced.columns = [product_id] + [f"feat_{i + 1}" for i in range(n_components)]
    X_test_reduced.columns = [product_id] + [f"feat_{i + 1}" for i in range(n_components)]
...
def build() -> None:
    ...
    X_train_grouped = created_train.merge(right=embed_train, how="left", on=params.product_id)

    X_test_grouped = created_test.merge(right=embed_test, how="left", on=params.product_id)

    X_train_scaled, X_test_scaled, sc = scaler(X_train_grouped, X_test_grouped, params.product_id)
    ...
    metadata["scaler"] = {"path": str(CORE_DIR / params.scale.artifact)}
    metadata["pca"] = {
