...
def _make_keras_mock(proba_output: np.ndarray) -> tuple[MagicMock, MagicMock]:
...
def loaded_predictor(settings: Settings) -> PredictorService:
    ...
    with (
        ...
        patch(
            "services.api.services.predictor.joblib.load",
            side_effect=[mock_scaler, mock_pca],
        ),
        patch(
            "services.api.services.predictor.SentenceTransformer",
...
class TestComputeStats:
    ...
    def test_empty_description_yields_zeros(self, loaded_predictor: PredictorService) -> None:
...
class TestChunkText:
    ...
    def test_empty_text_returns_placeholder(self, loaded_predictor: PredictorService) -> None:
...
class TestPredictBatch:
    def test_returns_one_result_per_input(self, loaded_predictor: PredictorService) -> None:
        loaded_predictor._model.predict.return_value = np.array([[0.2, 0.8], [0.9, 0.1]])
        inputs = [
    ...
    def test_batch_labels_match_argmax(self, loaded_predictor: PredictorService) -> None:
        loaded_predictor._model.predict.return_value = np.array([[0.2, 0.8], [0.9, 0.1]])
        inputs = [
            BasePredictionInput(designation="Product A"),
            BasePredictionInput(designation="Product B"),
